...
export class MdParser implements MarkdownParser {
  ...
  lex(md: string, curIndent: number = 0): MdNode[] {
    ...
    while (i < lines.length) {
      ...
      for (const NodeType of BLOCK_NODES) {
        ...
        if (node instanceof CodeBlock) {
          ...
          const getIndentLevel = (str: string) => {
          ...
          while ((match = ListBlock.regex.exec(lines[i]?.trim())) !== null) {
            ...
            while(j < lines.length && getIndentLevel(lines[j]) > curIndent){
              j++
            }

            if(j > i && i + 1 < lines.length){
              node.children = this.lex(lines.slice(i + 1, j).join('\n'), nextIndent)
            }
